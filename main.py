...
from corrupt_response import corrupt_response

SERVER = ''  # for all addresses
PORT = 53
DEBUG_MODE = False
zonefile = './corrupt.com.zone'
zone = dns.zone.from_file(zonefile, relativize=False)
CORRUPT_TEST_MODE = False

def process_command_line_args():
    global SERVER, PORT, DEBUG_MODE, zone, CORRUPT_TEST_MODE

    parser = argparse.ArgumentParser(
        description='This is a simple authoritative dns that returns a corrupted response.')
    ...
    parser.add_argument('-f', '--file',
                        help='The zone file to use. Default: "./corrupt.com.zone"')
    parser.add_argument('-c', '--corrupt', action='store_true',
                        help='Corrupt test mode')
    ...
    if args.corrupt:
        CORRUPT_TEST_MODE = True
        print('running in CORRUPT TEST MODE')
...
def handle_connection(sock, is_tcp):
    ...
    if CORRUPT_TEST_MODE:
        response, changed = corrupt_response(response, zone)
        if changed:
            print("\nResponse was altered as follows.")
            for key, (before, after) in changed.items():
                print('{}:\t{}\t->\t{}'.format(key, before, after))
            print(response)
...
def resolve_query(query):
    ...
    # Return NOTIMP for RR Types which this software does not support
